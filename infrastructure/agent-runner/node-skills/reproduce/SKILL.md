...
6. 未打出危害：允许重读源码、修正同一主张的利用链/编码/鉴权，然后继续 HTTP。判定即停。**变体预算**：同一核心主张最多试 5 个合理变体，每个变体一次主请求；超预算按当前最强证据判定，并在 `attempts` 里汇总"已试变体清单"供报告引用。
7. **文件写入 / 代码执行类主张的证明例外**：必须展示写入的内容经 HTTP 可访问或可执行（读回、回显、命令输出），仅服务端文件已存在/进程已变更不算确认。
8. 诊断证据不能作为已确认；确认证据必须是 HTTP 可观察危害。
9. SSRF 必须 preflight 验证回调通道可达。
10. `confirmed` / `partial` 必须交完整 `poc`（Python 脚本优先，bash 次之，其它须 `language_reason`）。`code` 必须是已打通请求的完整可运行源码，不是 demo。其余 verdict 不得交 poc。
