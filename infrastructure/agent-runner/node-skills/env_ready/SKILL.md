...
- `attempt > 1`：现有 `.vuln-env` 是上一轮产物。先读取它，再结合 `failed_stage` 和 `previous_error` 对症，**一次只改一处**。不要因为构建网络抖动而改启动拓扑。阶段含义：`compose_build`=镜像构建，`container_start`=进程启动，`container_healthcheck`=Dockerfile/compose healthcheck，`health_check`=平台 HTTP 正文探活，`compose_policy`=安全策略，`compose_timeout`=Compose 执行超时。
