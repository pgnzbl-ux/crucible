...
const RULES: [needle: string, title: string, hint: string][] = [
  ...
  ['超时', '历史节点超时记录', '当前版本已取消 Agent 总运行时长限制。重启 API 与 Celery worker 后，从本节点重试。'],
  ['源码解包失败', '上传源码解开失败', '核对源码包是否为 zip / tar.gz，以及任务是否仍能找到已上传的缓存。'],
  ['源码工作区准备失败', '源码工作目录权限异常', '旧靶场修改了源码属主，平台未能自动隔离目录。停止仍占用该任务目录的容器后，从源码获取节点重试。'],
  ['already exists and is not an empty directory', '源码工作目录没有清空', '这是旧版本遗留工作区导致的错误。更新并重启 Celery worker 后，从源码获取节点重试。'],
