...
轻量二审网关不持有工具、不读取仓库、不执行代码，只消费已裁剪文本。超时、解析失败和预算耗尽必须落为可观察状态并进入人工复核。
...
线索列表支持 task、project、状态、CWE、引擎、等级和 AI 结论筛选。详情返回代表发现完整证据、AI 二审、人工动作以及自动/人工终认摘要。人工确认和驳回必须记录原因；误报可复活。
