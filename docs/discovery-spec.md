...
- `api_inventory`：按画像 `languages` / `frameworks` 表驱动选择确定性 parser，产出 BOM + `resource_key` 索引；获取方式分 `router`（框架路由表）、`script_file`（PHP/Next pages 式文件入口）、`export_handler`（Next `route.ts` 导出方法）、`openapi`（OpenAPI/Swagger，跨语言恒跑）。PHP `script_file` 另用 tree-sitter 抽取请求传参面（超全局 + Laravel/ThinkPHP/CI/Yii 等常见封装白名单的字面量参数名）写入 `id_params`/`is_pve`；动态名与未知私有封装不保证。**禁止**用 AI 列端点；AI 不得把自拟 path 写入 BOM。parser 矩阵（按画像语言）：python=openapi+fastapi+flask+django；nodejs=openapi+express+nextjs+nestjs（Express 解析同时覆盖 Koa/Fastify 同形调用）；php=openapi+php_script+laravel；java=openapi+spring；go=openapi+gin（同时覆盖 Echo/Chi 同形调用）；rust/static/未知仅 openapi，无 parser 的语言列入 `unsupported_languages`，0 端点不得写成「无 API」。运行时拼接 path、反射注册、未解出的 `include_router` 前缀不进 BOM，只记文件座位，不让模型补全。
