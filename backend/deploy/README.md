...
cd /opt/crucible && sudo docker build -f backend/agent-runner/Dockerfile -t crucible-agent-runner:base .

# 4. 前端构建
cd /opt/crucible/frontend && npm ci && npm run build    # 产出 dist/

# 5. systemd 单元
sudo cp /opt/crucible/backend/deploy/crucible-api.service /etc/systemd/system/
sudo cp /opt/crucible/backend/deploy/crucible-worker.service /etc/systemd/system/
sudo systemctl daemon-reload
sudo systemctl enable --now crucible-api crucible-worker

# 6. Nginx
sudo cp /opt/crucible/backend/deploy/nginx-crucible.conf /etc/nginx/conf.d/crucible.conf
# 编辑 server_name / 证书路径
