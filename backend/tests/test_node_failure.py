...
@pytest.mark.parametrize(
    ...
    [
        ...
        ("compose_build", "COPY app /app failed", "compose_up.copy"),
        ("container_start", "container exited 1", "compose_up.runtime"),
        ("container_healthcheck", "container is unhealthy", "container_healthcheck"),
        ("compose_policy", "security policy", "compose_up.policy"),
        ("compose_timeout", "docker compose timeout", "compose_up.runtime"),
        ("docker_unavailable", "daemon down", "docker.unavailable"),
        ("health_check", "健康检查不过", "health_check"),
        (None, "AI 节点 env_ready 容器被 SIGKILL 终止(exit=137，多为任务取消、外部强杀或 OOM): x", "runner.killed"),
