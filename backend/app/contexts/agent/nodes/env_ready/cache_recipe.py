...
async def _try_cached_recipe(
    ...
    *,
    ...
    web_ports = ports.load_web_host_ports(str(compose_file))
    if not web_ports:
        return None, "缓存配方 compose 未把 Web 端口映射到宿主机。"
    ...
    ok, live_port, scheme = await health.health_check(
        web_ports, container_ports=ports.load_web_container_ports(str(compose_file))
    )
    ...
    published = publish_target_url(live_port, advertise, scheme=scheme)
    output = {
