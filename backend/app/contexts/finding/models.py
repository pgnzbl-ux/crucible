...
class RawFinding(BaseModel):
    """引擎原始告警(SARIF+ 归一化)。秘密值必须已脱敏(discovery-spec §8.2)。"""
...
class Adjudication(BaseModel):
    ...
    __table_args__ = (
        Index("idx_adjudications_group", "alert_group_id", "attempt"),
    )
    ...
    provider_id: Mapped[str | None] = mapped_column(String(36), ForeignKey("llm_providers.id"))
